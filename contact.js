...
const OWNER_EMAIL = 'hello@proxylet.com';
...
function populateProxyDropdown() {
  const select = document.getElementById('contactProxy');
  if (!select || typeof proxyListings === 'undefined') return;
  proxyListings.forEach(proxy => {
    const option       = document.createElement('option');
    option.value       = proxy.title;
    option.textContent = proxy.title + ' (' + proxy.country + ')';
...
function bindContactForm() {
  ...
  submitBtn.addEventListener('click', () => {
    ...
    const proxyTitle = document.getElementById('contactProxy').value;

    const emailSubject = 'ProxyLet Enquiry: ' + subject;
    ...
    const bodyLines = [
      ...
      message,
      '',
      '--- Sent via ProxyLet contact form ---'
